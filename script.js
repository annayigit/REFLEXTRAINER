window.addEventListener('DOMContentLoaded', () => { window.gameManager = new GameManager(); });
...
function bindTap(el, handler) {
  ...
  el.addEventListener('pointerdown', (e) => {
    ...
    if (locked) return;
    locked = true;
    try { handler(e); } finally { setTimeout(() => { locked = false; }, 180); }
  });
...
class GameManager {
  ...
  initEvents() {
    ...
    bindTap(document.getElementById('menu-btn'), () => { this.hideEndMenu(); this.showScreen('menu'); });

    bindTap(document.getElementById('inlevel-back-btn'), () => { this.timingStop(); this.showScreen('menu'); });
    ...
    bindTap(document.getElementById('end-restart-btn'), () => { this.hideEndMenu(); this.showLevelPrep(1); });
    bindTap(document.getElementById('end-to-menu-btn'), () => { this.hideEndMenu(); this.showScreen('menu'); });
    ...
    window.addEventListener('resize', () => { if (this._confetti) this.startConfetti(true); });
  }
  ...
  startCountdown() {
    ...
    const playStep = () => {
      ...
      if (val === 'GO!') { setTimeout(() => this.startGame(), 520); return; }
      i++;
  ...
  startGame() {
    ...
    document.getElementById('level-name').textContent = document.querySelector(`[data-level="${this.currentLevel}"]`).textContent.trim();
  ...
  timingStart() { if (!this._timing) { this._timing = true; this._t0 = performance.now(); } }
  timingStop() { if (this._timing) { this.timeSumMs += (performance.now() - this._t0); this._timing = false; } }
  markRound(correct) { if (correct) this.roundCorrect++; }
  ...
  finishLevel() {
    ...
    if (passed) {
      ...
      if (!Number.isFinite(prev) || total < prev) { times[key] = total; this.saveBestTimes(times); }
    }
    ...
    document.getElementById('results-stats').textContent = `Zeit: ${total} ms / ${maxMs} ms\nKorrekt: ${this.roundCorrect}/5`;
  ...
  startConfetti(keepExistingPieces) {
    ...
    const pieces = (keepExistingPieces && this._confetti && this._confetti.pieces)
  ...
  stopConfetti(clear = false) {
    const canvas = document.getElementById('confetti-canvas');
    if (this._confetti && this._confetti.raf) cancelAnimationFrame(this._confetti.raf);
...
class SimonSequence {
  ...
  start() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      panel.addEventListener('pointerdown', (e) => { e.preventDefault(); this.playerClick(i); });
...
class MemoryGame {
  ...
  nextRound() {
    ...
    values.forEach(v => {
      ...
      card.addEventListener('pointerdown', (e) => { e.preventDefault(); this.enqueue(() => this.flip(card)); });
      grid.appendChild(card);
    });

    const closedBeforeRevealMs = 220;
    const revealMsByRound = [900, 800, 850, 900, 2600];
    const revealMs = revealMsByRound[this.round - 1] ?? 900;
  ...
  async flip(card) {
    ...
    if (a === b) {
      ...
      setTimeout(() => {
        if (this.first) this.first.classList.remove('just-matched');
        if (this.second) this.second.classList.remove('just-matched');
      }, 200);
